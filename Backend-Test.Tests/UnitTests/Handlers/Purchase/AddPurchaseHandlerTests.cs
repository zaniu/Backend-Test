...
public class AddPurchaseHandlerTests
{
    ...
    public async Task Handle_WithNewId_AddsPurchase()
    {
        ...
        // Act
        var response = await handler.Handle(new AddPurchaseRequest(999, 1,
        [
        ...
        repositoryMock.Verify(repository => repository.TryAdd(It.Is<Model.Purchase>(purchase =>
            ...
            purchase.Items.Any(item => item.ProductId == 1 && item.Count == 1) &&
            purchase.Items.Any(item => item.ProductId == 2 && item.Count == 1)), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_WithDuplicateId_ThrowsException()
    {
        ...
        // Act
        var act = async () => await handler.Handle(new AddPurchaseRequest(1, 1, [new AddPurchaseRequest.PurchaseItem(1, 1)]), CancellationToken.None);
    ...
    public async Task Handle_WithMissingCustomer_ThrowsException()
    {
        ...
        // Act
        var act = async () => await handler.Handle(new AddPurchaseRequest(1000, 999, [new AddPurchaseRequest.PurchaseItem(1, 1)]), CancellationToken.None);
    ...
    public async Task Handle_WithMissingProduct_ThrowsException()
    {
        ...
        // Act
        var act = async () => await handler.Handle(new AddPurchaseRequest(1001, 1, [new AddPurchaseRequest.PurchaseItem(999, 1)]), CancellationToken.None);
